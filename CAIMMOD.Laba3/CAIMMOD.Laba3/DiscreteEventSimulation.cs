...
public class DiscreteEventSimulation
{
    ...
    private readonly List<(double Time, double Duration)> _customerHistory;
    ...
    public SimulationResult Run()
    {
        ...
        // Когда в последний раз вычислялась загрузка
        var previousTime = 0.0;
        // Предыдущее значение загрузки
        var prevUtil = 0.0;
    ...
    private void HandleArrival(double now)
    {
        ...
        _traceLog?.Invoke($"Прибытие группы {group.Id} (size={group.Size}) в t={now:F3}");

        var freeTable = _tables.FirstOrDefault(t => !t.IsOccupied);
        if (freeTable == null)
        {
            _totalLost++;
            _traceLog?.Invoke($"Группа {group.Id} ушла — нет свободных столов");
        }
        else
        {
            freeTable.IsOccupied = true;
            freeTable.OccupyingGroup = group;
            _traceLog?.Invoke($"Группа {group.Id} заняла стол {freeTable.Id} в t={now:F3}");
            ...
            if (freeWaiter != null)
            {
                ...
                _traceLog?.Invoke($"Официант {freeWaiter.Id} начал обслуживание группы {group.Id} в t={now:F3}, длительность {serviceTime:F3}");
            }
            else
            {
                if (SimulationParameters.WaitIfNoWaiter)
                {
                    _traceLog?.Invoke($"Группа {group.Id} ожидает официанта у стола {freeTable.Id}");
                }
                else
                {
                    ...
                    _totalLost++;
                    _traceLog?.Invoke($"Группа {group.Id} ушла — политика не позволяет ждать официанта");
                }
            }
        }
    ...
    private void HandleServiceEnd(Event e, double now)
    {
        ...
        if (e.Group != null)
        {
            var timeInSystem = now - e.Group.ArrivalTime;
            _customerHistory.Add((now, timeInSystem));
        }

        if (waiter == null || group == null) return;
        _traceLog?.Invoke($"Официант {waiter.Id} закончил обслуживание группы {group.Id} в t={now:F3}");
        ...
        if (waitingTable != null)
        {
            ...
            _traceLog?.Invoke($"Официант {waiter.Id} начал обслуживание ожидающей группы {occupyingGroup.Id} в t={now:F3}, длительность {serviceTime:F3}");
        }
        else
        {
            _traceLog?.Invoke($"Официант {waiter.Id} свободен в t={now:F3}");
        }
    }
    ...
    private void HandleServiceStartWaiting(Event e, double now)
    {
        ...
        {
            _traceLog?.Invoke($"ServiceStartWaiting для группы {group.Id} в t={now:F3} проигнорирован — группа уже обслуживается");
            return;
        }
        ...
        if (freeWaiter != null)
        {
            ...
            if (table == null)
            {
                _traceLog?.Invoke($"ServiceStartWaiting: группа {group.Id} больше не за столом в t={now:F3}");
                return;
            }
            ...
            var end = now + serviceTime;
            _eventQueue.Enqueue(new Event(end, EventType.ServiceEnd, group, freeWaiter), end);
            _traceLog?.Invoke($"(ServiceStartWaiting) Официант {freeWaiter.Id} начал обслуживание группы {group.Id} в t={now:F3}, длительность {serviceTime:F3}");
        }
        else
        {
            _traceLog?.Invoke($"ServiceStartWaiting: в t={now:F3} свободного официанта не найдено для группы {group.Id}");
        }
    }
}